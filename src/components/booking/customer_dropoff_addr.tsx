...
function AddressForm() {
  const { formData, validationErrors, setCustomerDetails,setValidationErrors } = useBookingStore();
  ...
  const debouncedSearchState = useCallback(
    debounce(async () => {
      ...
      try {
        ...
        setCustomerDetails({state_id: filteredData[0].id})
        setCustomerDetails({state: filteredData[0].name})
  ...
  useEffect(() => {
    if (!formData?.customer?.isAutofilled) {
      ...
      setCustomerDetails({
        full_address: formData?.customer?.full_address,
        city_id: formData?.customer?.city_id,
        area: formData?.customer?.area,
        pincode: formData?.customer?.pincode,
        landmark: formData?.customer?.landmark,
        street: formData?.customer?.street,
        location: formData?.customer?.location,
        flat: formData?.customer?.flat,
      ...
      try {
        const response = await fetch(
          `https://apis.nexifyworld.com/api/commons/get-cities`,
          {
            ...
            body: JSON.stringify({ state_id: formData?.customer?.state_id }),
          }
        );
  ...
  const handleSearchLocation = async (
    ...
    formData.customer.full_address = value
  ...
  const handleSelectLocation = async (prediction: any) => {
    ...
    validationErrors["customer.full_address"] = "";
    ...
    try {
      ...
      formData.customer.full_address = prediction.text.text;
      formData.customer.location.lat = data.location.latitude;
      formData.customer.location.lng = data.location.longitude;
    } catch (error) {
      console.error("Error fetching predictions:", error);
    }
  };
  ...
  const handleCityInput = (event: any) => {
    ...
    formData.customer.city_id = cityId;
    validationErrors["customer.city"] = "";
  }
  ...
  const handleAddressCheckbox = async (e: any) => {
    ...
    if (e.target.checked == false) {
      ...
      if (customerLicense) {
        ...
        setCustomerDetails(updatedPickupDetails);
      ...
      setValidationErrors((prevErrors) => {
        ...
        delete newErrors['customer.full_address'];
        delete newErrors['customer.city_id'];
        delete newErrors['customer.area'];
        delete newErrors['customer.pincode'];
        delete newErrors['customer.landmark'];
        delete newErrors['customer.street'];
        delete newErrors['customer.flat'];
        delete newErrors['customer.location'];
        ...
        <div className="bg-white border-[1.5px] p-4 border-borderColor rounded-lg">
          <div className="grid gap-3">
            <div className="flex flex-col gap-2">
              ...
              <div className="relative w-full w-full">
                <input
                  ...
                  value={formData.customer.isAutofilled ? formData.customer.full_address : inputValue}
              ...
              ${validationErrors["customer.full_address"] ? "border-red-500" : ""}`}
              ...
              {validationErrors["customer.full_address"] && (
                <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.full_address"]}</span>
              )}
            </div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-1 gap-6">
              ...
              ${validationErrors["customer.flat"] ? "border-red-500" : ""}`}
                  placeholder="Enter House / Flat / Floor"
                  defaultValue={formData.customer.isAutofilled ? formData.customer.flat : ""}
                  onChange={(e) => {
                    validationErrors["customer.flat"] = "";
                  setCustomerDetails({flat:e.target.value})
                  }}
                ...
                />
                {validationErrors["customer.flat"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.flat"]}</span>
                )}
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  name="street"
                  className={`border-[1.5px] rounded-[6px] h-[38px] text-base placeholder:text-[#CCBEBE] font-medium border-borderColor px-3 py-2 transition duration-300 ease focus:outline-none focus:border-primary hover:border-primary
              ${validationErrors["customer.street"] ? "border-red-500" : ""}`}
                  placeholder="Enter Street"
                  defaultValue={formData.customer.isAutofilled ? formData.customer.street : ""}
                  onChange={(e) => {
                    validationErrors["customer.street"] = "";
                    setCustomerDetails({street:e.target.value})
                ...
                />
                {validationErrors["customer.street"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.street"]}</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  name="area"
                  className={`border-[1.5px] rounded-[6px] h-[38px] text-base placeholder:text-[#CCBEBE] font-medium border-borderColor px-3 py-2 transition duration-300 ease focus:outline-none focus:border-primary hover:border-primary
              ${validationErrors["customer.area"] ? "border-red-500" : ""}`}
                  placeholder="Enter Area"
                  defaultValue={formData.customer.isAutofilled ? formData.customer.area : ""}
                  onChange={(e) => {
                    validationErrors["customer.area"] = "";
                    setCustomerDetails({area:e.target.value})
                ...
                />
                {validationErrors["customer.area"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.area"]}</span>
                )}
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-sm text-textLightColor font-medium">
                ...
                <input
                  ...
                  name="land_mark"
                  className={`border-[1.5px] rounded-[6px] h-[38px] text-base placeholder:text-[#CCBEBE] font-medium border-borderColor px-3 py-2 transition duration-300 ease focus:outline-none focus:border-primary hover:border-primary
              ${validationErrors["customer.landmark"] ? "border-red-500" : ""}`}
                  placeholder="Enter Land Mark"
                  defaultValue={formData.customer.isAutofilled ? formData.customer.landmark : ""}
                  onChange={(e) => {
                    validationErrors["customer.landmark"] = "";
                ...
                />
                {validationErrors["customer.landmark"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.landmark"]}</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  type="number"
                  className={`border-[1.5px] rounded-[6px] h-[38px] text-base placeholder:text-[#CCBEBE] font-medium border-borderColor px-3 py-2 transition duration-300 ease focus:outline-none focus:border-primary hover:border-primary
                  ${validationErrors["customer.pincode"] ? "border-red-500" : ""}`}
                  placeholder="Enter Pincode"
                  defaultValue={formData.customer.isAutofilled ? formData.customer.pincode : ""}
                  onChange={(e) => {
                    setValidationErrors((prevErrors) => {
                      const newErrors = { ...prevErrors };
                      delete newErrors['customer.pincode']; // Remove the error
                      return newErrors;
                    });

                    setCustomerDetails({pincode:e.target.value})
                ...
                {validationErrors["customer.pincode"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.pincode"]}</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                {validationErrors["customer.state"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.state"]}</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="w-full w-full">
                  ...
                  <select
                    ...
                    className={`border-[1.5px] h-[38px] text-[16px] placeholder:font-medium placeholder:text-[#CCBEBE] font-medium border-borderColor rounded-[6px] px-3 py-2 w-full focus:outline-none focus:border-primary hover:border-primary ${validationErrors["customer.state"] ? "border-red-500" : ""
                      }`}
                  ...
                  >
                    <option value=""> {
                      ...
                      formData.customer.city == "" ? "Select City" : formData.customer.city}</option>
                ...
                {validationErrors["customer.city_id"] && (
                  <span className="error text-[12px] text-red-500 ">❗{validationErrors["customer.city_id"]}</span>
                )}
              </div>
            </div>
          </div>
